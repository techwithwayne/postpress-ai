...
require_once __DIR__ . '/inc/class-postpress-ai-remote-drafts.php';

// Enqueue remote drafts script on PostPress AI admin screens.
if ( ! function_exists( 'postpress_ai_enqueue_remote_drafts_script' ) ) {
	function postpress_ai_enqueue_remote_drafts_script( $hook ) {
		// Only load on PostPress AI screens (hook contains "postpress-ai").
		if ( strpos( $hook, 'postpress-ai' ) === false ) {
			return;
		}

		// Bail out if constants aren't defined for any reason.
		if ( ! defined( 'PPA_PLUGIN_URL' ) || ! defined( 'PPA_VERSION' ) ) {
			return;
		}
	...
	add_action( 'admin_enqueue_scripts', 'postpress_ai_enqueue_remote_drafts_script' );
}
