 ...
 *
 * CHANGE LOG
 * 2026-01-04 • OPTION A: Customer domains do NOT require shared key. Inject license_key+site_url into payload for preview/generate/store and let Django enforce. // CHANGED:
 * 2026-01-04 • OPTION A: If shared key missing and no explicit blocker, WP gate does NOT block (Django remains authoritative). // CHANGED:
 * 2026-01-04 • OPTION A: Stop sending license key as X-PPA-Key; send X-PPA-Key only when shared key exists. // CHANGED:
 * 2026-01-04 • OPTION A: Add auth-parity headers for customer mode: X-PPA-License-Key + X-PPA-Site-Url (supports Django Option A + GET debug endpoints). // CHANGED:
 * 2026-01-04 • FIX: Prefer HTTPS site_url when admin request is HTTPS (prevents activation mismatch vs Django stored https). // CHANGED:
 *
 * 2026-01-04 • FIX: Gate prefers explicit opt:ppa_license_last_error_code blockers (site_limit_reached, not_activated, etc.) over "unknown/insufficient_data". // CHANGED:
...
if ( ! class_exists( 'PPA_Controller' ) ) {
	...
	class PPA_Controller {
		...
		/**
		 * Track which auth mode was chosen for the current request (shared|payload). // CHANGED:
		 *
		 * @var string
		 */
...
if ( $enforce_gate ) { // CHANGED:
		...
		private static function payload_inject_license_and_site( $payload ) { // CHANGED:
			...
			if ( ! isset( $json['site_url'] ) || ! is_string( $json['site_url'] ) || trim( (string) $json['site_url'] ) === '' ) { // CHANGED:
				$json['site_url'] = self::current_site_url_full(); // CHANGED:
			} // CHANGED:

			$raw = function_exists( 'wp_json_encode' ) ? wp_json_encode( $json ) : json_encode( $json ); // CHANGED:
			$raw = is_string( $raw ) ? $raw : '{}'; // CHANGED:

			$payload['json'] = $json; // CHANGED:
			$payload['raw']  = $raw;  // CHANGED:
		...
		// NOTE: proxy license-header capability helpers kept for backward compat, but Option A no longer uses license-in-header. // CHANGED:
		...
		private static function proxy_auth_key_or_500() { // CHANGED:
			// OPTION A: only send X-PPA-Key when a shared key exists. Otherwise we rely on payload license_key+site_url. // CHANGED:
			...
			if ( '' !== $license ) { // CHANGED:
				self::$proxy_auth_mode = 'payload'; // CHANGED:
				return ''; // CHANGED:
			} // CHANGED:
		...
		private static function learn_or_block_license_proxy_auth( $http_code, $json ) { // CHANGED:
			// Option A no longer uses license-in-header, so only learn/block for the legacy mode. // CHANGED:
		...
		private static function persist_license_truth( $truth ) { // CHANGED:
			...
			if ( 'active' === $state ) { // CHANGED:
				...
				update_option( 'ppa_license_active_site', self::current_site_url_full(), false ); // CHANGED:
		...
		private static function explicit_blocker_from_options() { // CHANGED:
			$raw = get_option( 'ppa_license_last_error_code', '' ); // CHANGED:
			if ( ! is_string( $raw ) ) { return ''; } // CHANGED:
			$code = sanitize_key( trim( (string) $raw ) ); // CHANGED:
			if ( '' === $code || 'unknown' === $code ) { return ''; } // CHANGED:

			$explicit = array( // CHANGED:
				'site_limit_reached', // CHANGED:
				'not_activated',      // CHANGED:
				'inactive',           // CHANGED:
				'expired',            // CHANGED:
				'invalid_license',    // CHANGED:
				'unauthorized',       // CHANGED:
				'forbidden',          // CHANGED:
				'revoked',            // CHANGED:
				'cancelled',          // CHANGED:
				'payment_required',   // CHANGED:
			); // CHANGED:

			return in_array( $code, $explicit, true ) ? $code : ''; // CHANGED:
		} // CHANGED:
		...
		private static function server_verify_license_truth() { // CHANGED:
			...
			$shared = self::shared_key(); // CHANGED:
			if ( '' === $shared ) { // CHANGED:
				// OPTION A: without shared key we skip server verify here; Django will enforce on content endpoints. // CHANGED:
				return array( 'state' => 'unknown', 'reason' => 'shared_missing', 'source' => 'server' ); // CHANGED:
			} // CHANGED:
			...
			$payload = array( // CHANGED:
				...
				'site_url'    => self::current_site_url_full(), // CHANGED:
			); // CHANGED:
		...
		private static function build_args( $raw_json ) {
			...
			// OPTION A: only set X-PPA-Key when shared key exists. // CHANGED:
			$k = self::proxy_auth_key_or_500(); // CHANGED:
			if ( is_string( $k ) && $k !== '' ) { // CHANGED:
				$headers['X-PPA-Key'] = $k; // CHANGED:
			} // CHANGED:

			// OPTION A: auth-parity headers (do NOT use X-PPA-Key for license key). // CHANGED:
			$site_full = self::current_site_url_full(); // CHANGED:
			if ( $site_full !== '' ) { // CHANGED:
		...
		public static function ajax_preview() {
			...
			$payload = self::payload_inject_license_and_site( self::read_json_body() ); // CHANGED:
