...
if ( ! class_exists( 'PPA_Controller' ) ) {
	...
	class PPA_Controller {
		...
		private static function verify_nonce_or_forbid() {
			...
			// 2) Fallback: accept nonce from common POST/REQUEST fields (matches admin-account.js)  // CHANGED:
			if ( '' === $nonce ) { // CHANGED:
					$candidates = array( 'nonce', '_ajax_nonce', '_wpnonce', 'security', 'ppa_nonce' ); // CHANGED:
					foreach ( $candidates as $k ) { // CHANGED:
							if ( isset( $_POST[ $k ] ) ) { // CHANGED:
									$nonce = trim( (string) wp_unslash( $_POST[ $k ] ) ); // CHANGED:
									if ( '' !== $nonce ) { // CHANGED:
											break; // CHANGED:
									} // CHANGED:
							} // CHANGED:
							if ( isset( $_REQUEST[ $k ] ) ) { // CHANGED:
									$nonce = trim( (string) wp_unslash( $_REQUEST[ $k ] ) ); // CHANGED:
									if ( '' !== $nonce ) { // CHANGED:
											break; // CHANGED:
									} // CHANGED:
							} // CHANGED:
			...
			// 3) Validate (non-fatal / no "-1" dies; we return JSON 403 consistently)             // CHANGED:
			$ok = false; // CHANGED:
			if ( $nonce ) { // CHANGED:
					$ok = (bool) wp_verify_nonce( $nonce, 'ppa-admin' ); // CHANGED:
			} // CHANGED:
