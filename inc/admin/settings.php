...
if ( ! class_exists( 'PPA_Admin_Settings' ) ) {
        ...
        class PPA_Admin_Settings {
                ...
                private static function notice_from_license_result( $label, $result ) {
                        ...
                        // CHANGED: Prefer the upstream error message (best customer/admin UX).
                        $msg = '';
                        if ( is_array( $result ) ) {
                                if ( isset( $result['error']['message'] ) && is_string( $result['error']['message'] ) ) {
                                        $msg = trim( $result['error']['message'] );
                                } elseif ( isset( $result['error']['hint'] ) && is_string( $result['error']['hint'] ) ) {
                                        $msg = trim( $result['error']['hint'] );
                                } elseif ( isset( $result['meta']['message'] ) && is_string( $result['meta']['message'] ) ) {
                                        $msg = trim( $result['meta']['message'] );
                                } elseif ( isset( $result['meta']['reason'] ) && is_string( $result['meta']['reason'] ) ) {
                                        $msg = trim( $result['meta']['reason'] );
                                }
                        }

                        if ( strlen( $msg ) > 260 ) { // keep banners readable (no walls of text).
                                $msg = substr( $msg, 0, 260 ) . '…';
                        }

                        // CHANGED: Prefer stable codes from meta.error_code, then error.code/type.
                        $code = '';
                        if ( is_array( $result ) && isset( $result['meta']['error_code'] ) ) {
                                $code = (string) $result['meta']['error_code'];
                        } elseif ( is_array( $result ) && isset( $result['error']['code'] ) ) {
                                $code = (string) $result['error']['code'];
                        } elseif ( is_array( $result ) && isset( $result['meta']['error_type'] ) ) {
                                $code = (string) $result['meta']['error_type'];
                        } elseif ( is_array( $result ) && isset( $result['error']['type'] ) ) {
                                $code = (string) $result['error']['type'];
                        } elseif ( is_array( $result ) && isset( $result['meta']['http_status'] ) ) {
                                $code = 'http_' . (string) (int) $result['meta']['http_status'];
                        } elseif ( is_array( $result ) && isset( $result['_http_status'] ) ) {
                                $code = 'http_' . (string) (int) $result['_http_status'];
                        }
                ...
                public static function render_page() {
                        ...
                        $status_cls = ( isset( $banner['type'] ) && 'ok' === (string) $banner['type'] ) ? 'is-good' : 'is-bad';

                        $url_account  = admin_url( 'admin.php?page=postpress-ai-account' );
                        $url_composer = admin_url( 'admin.php?page=postpress-ai' );

                        $status_label = __( 'Unknown', 'postpress-ai' );
                        if ( $is_active_here ) {
                                $status_label = __( 'Active on this site', 'postpress-ai' );
                        } elseif ( $is_inactive_here ) {
                                $status_label = __( 'Not active', 'postpress-ai' );
                        } elseif ( ! $has_key ) {
                                $status_label = __( 'Not set up yet', 'postpress-ai' );
                        }
                        ...
                        <div class="wrap ppa-admin ppa-settings">
                                <div class="ppa-header">
                                        <div class="ppa-header__left">
                                                <h1 class="ppa-title"><?php esc_html_e( 'PostPress AI Settings', 'postpress-ai' ); ?></h1>
                                                <p class="ppa-subtitle"><?php esc_html_e( 'Connect your license and toggle activation for this site.', 'postpress-ai' ); ?></p>
                                        </div>
                                        <div class="ppa-header__right">
                                                <a class="button ppa-btn ppa-btn--solid" href="<?php echo esc_url( $url_account ); ?>">
                                                        <?php esc_html_e( 'Open Account', 'postpress-ai' ); ?>
                                                </a>
                                                <a class="button ppa-btn ppa-btn--outline" href="<?php echo esc_url( $url_composer ); ?>">
                                                        <?php esc_html_e( 'Open Composer', 'postpress-ai' ); ?>
                                                </a>
                                        </div>
                                </div>

                                <div id="ppa-settings-status" class="ppa-status <?php echo esc_attr( $status_cls ); ?>" role="status" aria-live="polite">
                                        <span class="ppa-status__dot" aria-hidden="true"></span>
                                        <span class="ppa-status__text"><?php echo esc_html( isset( $banner['msg'] ) ? (string) $banner['msg'] : '' ); ?></span>
                                </div>
                                ...
                                <div class="ppa-grid">
                                        ...
                                        <div class="ppa-card ppa-card--setup">
                                                <div class="ppa-card__head">
                                                        <h2 class="ppa-card__title"><?php esc_html_e( 'Setup', 'postpress-ai' ); ?></h2>
                                                        <span class="ppa-pill" data-state="<?php echo esc_attr( $is_active_here ? 'active' : ( $is_inactive_here ? 'inactive' : 'unknown' ) ); ?>">
                                                                <?php echo esc_html( $status_label ); ?>
                                                        </span>
                                                </div>

                                                <p class="ppa-muted">
                                                        <?php esc_html_e( 'Paste your license key, then click Save. Use “Check License” to refresh server status.', 'postpress-ai' ); ?>
                                                </p>

                                                <?php if ( ! empty( $ck['present'] ) ) : ?>
                                                        <div class="ppa-note" role="note">
                                                        ...
                                                        <dl class="ppa-kv ppa-kv--form">
                                                                <div class="ppa-kv__row">
                                                                        ...
                                                                        <dd>
                                                                                ...
                                                                                />
                                                                                <div class="ppa-muted ppa-muted--tight">
                                                                                        <?php esc_html_e( 'Leave blank to keep your current key. Paste a new key to replace it.', 'postpress-ai' ); ?>
                                                                                </div>

                                                                                <?php if ( $has_key ) : ?>
                                                                                        <label class="ppa-checkbox">
                                                                                                <input type="checkbox" name="ppa_license_key_clear" value="1" />
                                                                                                <span><?php esc_html_e( 'Remove saved key when I click Save', 'postpress-ai' ); ?></span>
                                                                                        </label>
                                                ...
                                                <div class="ppa-actions">
                                                        <button type="submit" class="button ppa-btn ppa-btn--solid" form="ppa-settings-form">
                                                                <?php esc_html_e( 'Save', 'postpress-ai' ); ?>
                                                        </button>

                                                        <form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" class="ppa-inline-form">
                                                                <?php wp_nonce_field( 'ppa-license-verify' ); ?>
                                                                <input type="hidden" name="action" value="ppa_license_verify" />
                                                                <?php
                                                                $disable_verify = ( ! $has_key );
                                                                ?>
                                                                <button type="submit" class="button ppa-btn ppa-btn--outline" <?php echo $disable_verify ? 'disabled="disabled"' : ''; ?>>
                                                                        <?php esc_html_e( 'Check License', 'postpress-ai' ); ?>
                                                                </button>

                                                                <?php if ( $disable_verify ) : ?>
                                                                        <div class="ppa-muted ppa-muted--tight"><?php esc_html_e( 'Save your license key first.', 'postpress-ai' ); ?></div>
                                                                <?php elseif ( ! $is_active_here ) : ?>
                                                                        <div class="ppa-muted ppa-muted--tight"><?php esc_html_e( 'Next step: toggle on in Connection to activate this site.', 'postpress-ai' ); ?></div>
                                                                <?php endif; ?>
                                                        </form>
                                                </div>
                                                ...
                                                <?php if ( $has_key ) : ?>
                                                        ...
                                                        ?>
                                                        <div class="ppa-actions ppa-actions--secondary">
                                                                <a class="button ppa-btn ppa-btn--ghost" href="<?php echo esc_url( $reveal_url ); ?>">
                                                                        <?php esc_html_e( 'Reveal key (30s)', 'postpress-ai' ); ?>
                                                                </a>
                                                                <a class="button ppa-btn ppa-btn--ghost ppa-btn--danger" href="<?php echo esc_url( $clear_url ); ?>">
                                                                        <?php esc_html_e( 'Remove key', 'postpress-ai' ); ?>
                                                                </a>
                                                        </div>
                                                        ...
                                                        ?>
                                                                <div class="ppa-note" role="note">
                                                                        <strong><?php esc_html_e( 'Full key (visible for 30s):', 'postpress-ai' ); ?></strong>
                                        ...
                                        <!-- CONNECTION -->
                                        <div class="ppa-card ppa-card--conn">
                                                <div class="ppa-card__head">
                                                        <h2 class="ppa-card__title"><?php esc_html_e( 'Connection', 'postpress-ai' ); ?></h2>
                                                        <?php if ( $is_active_here ) : ?>
                                                                <span class="ppa-pill" data-state="active"><?php esc_html_e( 'Active on this site', 'postpress-ai' ); ?></span>
                                                        <?php elseif ( $is_inactive_here ) : ?>
                                                                <span class="ppa-pill" data-state="inactive"><?php esc_html_e( 'Not active', 'postpress-ai' ); ?></span>
                                                        <?php else : ?>
                                                                <span class="ppa-pill" data-state="unknown"><?php esc_html_e( 'Unknown', 'postpress-ai' ); ?></span>
                                                        <?php endif; ?>
                                                </div>

                                                <p class="ppa-muted">
                                                        <?php esc_html_e( 'Use the toggle to activate or deactivate this site.', 'postpress-ai' ); ?>
                                                </p>

                                                <div class="ppa-toggle-row">
                                                        <div class="ppa-toggle-row__left">
                                                                <div class="ppa-toggle-row__label"><?php esc_html_e( 'Connected', 'postpress-ai' ); ?></div>
                                                                <div class="ppa-toggle-row__hint"><?php esc_html_e( 'Turns PostPress AI on for this site.', 'postpress-ai' ); ?></div>
                                                        </div>

                                                        <div class="ppa-toggle-row__right">
                                                                <span class="ppa-toggle-row__state" id="ppa-connected-state"><?php echo $is_active_here ? esc_html__( 'On', 'postpress-ai' ) : esc_html__( 'Off', 'postpress-ai' ); ?></span>
                                                                <label class="ppa-switch">
                                                                        <input type="checkbox" id="ppa-connected-toggle" <?php echo $is_active_here ? 'checked="checked"' : ''; ?> <?php echo ( ( ! $has_key ) || ( $site_limit_reached && ! $is_active_here ) ) ? 'disabled="disabled"' : ''; ?> />
                                                                        <span class="ppa-slider" aria-hidden="true"></span>
                                                                </label>
                                                        </div>
                                                </div>

                                                <?php if ( ! $has_key ) : ?>
                                                        <p class="ppa-muted ppa-muted--tight"><?php esc_html_e( 'Save your license key first.', 'postpress-ai' ); ?></p>
                                                <?php elseif ( $site_limit_reached ) : ?>
                                                        <p class="ppa-muted ppa-muted--tight"><?php esc_html_e( 'Plan limit reached. Upgrade your plan or deactivate another site, then try again.', 'postpress-ai' ); ?></p>
                                                <?php endif; ?>

                                                <form id="ppa-activate-form" method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" class="ppa-hidden">
                                                        <?php wp_nonce_field( 'ppa-license-activate' ); ?>
                                                        <input type="hidden" name="action" value="ppa_license_activate" />
                                                </form>

                                                <form id="ppa-deactivate-form" method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" class="ppa-hidden">
                                                ...
                                                <div class="ppa-last-response-head">
                                                        <h3 class="ppa-h3"><?php esc_html_e( 'Last response (optional)', 'postpress-ai' ); ?></h3>
                                                        <span class="ppa-copy-feedback" id="ppa-copy-feedback" aria-live="polite"><?php esc_html_e( 'Copied', 'postpress-ai' ); ?></span>
                                                </div>
                                                <p class="ppa-muted ppa-muted--tight">
                                                        <?php esc_html_e( 'Only for troubleshooting. Click inside to copy. Click “Check License” to refresh.', 'postpress-ai' ); ?>
                                                </p>

                                                <textarea readonly id="ppa-last-response" class="ppa-debug-box"><?php
                                                        echo esc_textarea( $last ? wp_json_encode( $last, JSON_PRETTY_PRINT ) : 'No recent result yet.' );
                                                ?></textarea>

                                                <script>
                                                (function(){
                                                        var toggle = document.getElementById('ppa-connected-toggle');
                                                        var stateEl = document.getElementById('ppa-connected-state');
                                                        var formOn  = document.getElementById('ppa-activate-form');
                                                        var formOff = document.getElementById('ppa-deactivate-form');
                                                        var box     = document.getElementById('ppa-last-response');
                                                        var fb      = document.getElementById('ppa-copy-feedback');

                                                        function setStateLabel(on){
                                                                if (!stateEl) return;
                                                                stateEl.textContent = on ? 'On' : 'Off';
                                                        }

                                                        if (toggle && stateEl) {
                                                                setStateLabel(!!toggle.checked);

                                                                toggle.addEventListener('change', function(){
                                                                        var on = !!toggle.checked;
                                                                        setStateLabel(on);

                                                                        // Guard: don’t fire if missing forms.
                                                                        if (on && formOn)  { formOn.submit(); }
                                                                        if (!on && formOff) { formOff.submit(); }
                                                                });
                                                        }

                                                        // Last response: click/focus auto-copy (silent).
                                                        function flashCopied(){
                                                                if (!fb) return;
                                                                fb.classList.add('is-on');
                                                                window.setTimeout(function(){ fb.classList.remove('is-on'); }, 900);
                                                        }
                                                        ...
                                                        if (box) {
                                                                ...
                                                                box.addEventListener('focus', handler);
                                        ...
                                        <div class="ppa-card ppa-card--test">
                                                <div class="ppa-card__head">
                                                        <h2 class="ppa-card__title"><?php esc_html_e( 'Test Connection', 'postpress-ai' ); ?></h2>
                                                </div>
                                                <p class="ppa-muted"><?php esc_html_e( 'Click to make sure this site can reach PostPress AI.', 'postpress-ai' ); ?></p>
                                                ...
                                                <form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" class="ppa-inline-form">
                                                        ...
                                                        ?>
                                                        <button type="submit" class="button ppa-btn ppa-btn--outline" <?php echo $disable_test ? 'disabled="disabled"' : ''; ?>>
                                                                <?php esc_html_e( 'Test Connection', 'postpress-ai' ); ?>
                                                        </button>
                                                        <?php if ( $disable_test ) : ?>
                                                                <div class="ppa-muted ppa-muted--tight"><?php esc_html_e( 'Save your license key first.', 'postpress-ai' ); ?></div>
                                                        <?php endif; ?>
