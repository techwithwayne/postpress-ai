 ...
 *             - Conditionally show the “tokens/account links” note only when relevant
 *             - Guard local seed from pretending it’s a full Check License payload. // CHANGED:
 *
...
if ( ! class_exists( 'PPA_Admin_Settings' ) ) {
	...
	class PPA_Admin_Settings {
		...
		private static function derive_activation_state( $last ) {                               // CHANGED:
			...
			// CHANGED: Fall back to persisted state even if the transient expired or was never set.
			$persisted = get_option( self::OPT_LICENSE_STATE, 'unknown' );                        // CHANGED:
			$persisted = is_string( $persisted ) ? strtolower( trim( $persisted ) ) : 'unknown';  // CHANGED:
			if ( in_array( $persisted, array( 'active', 'inactive' ), true ) ) {                  // CHANGED:
		...
		private static function seed_last_license_transient_if_missing( $last ) {                // CHANGED:
			...
			$state = get_option( self::OPT_LICENSE_STATE, 'unknown' );                            // CHANGED:
			$state = is_string( $state ) ? strtolower( trim( $state ) ) : 'unknown';             // CHANGED:
			if ( ! in_array( $state, array( 'active', 'inactive', 'unknown' ), true ) ) {         // CHANGED:
			...
			if ( '' !== $stored_active && $stored_active === $home ) {                            // CHANGED:
				$activated = true;                                                               // CHANGED:
			} elseif ( 'active' === $state ) {                                                   // CHANGED:
				$activated = true;                                                               // CHANGED:
			}                                                                                    // CHANGED:

			$masked = self::mask_secret( (string) get_option( self::OPT_LICENSE_KEY, '' ) );      // CHANGED:

			$seed = array(                                                                        // CHANGED:
				'ok'           => ( '' === $err && in_array( $state, array( 'active', 'inactive' ), true ) ), // CHANGED:
				'data'         => array(                                                          // CHANGED:
					'license'    => array(                                                         // CHANGED:
						'status'     => $state,                                                    // CHANGED:
						'key_masked' => $masked,                                                   // CHANGED:
					),                                                                             // CHANGED:
					'activation' => array(                                                         // CHANGED:
						'site_url'         => $home,                                               // CHANGED:
						'activated'        => (bool) $activated,                                   // CHANGED:
						'last_verified_at' => $checked_iso,                                        // CHANGED:
						'_source'          => 'local_snapshot',                                    // CHANGED:
					),                                                                             // CHANGED:
				),                                                                                // CHANGED:
			...
			if ( '' !== $err ) {                                                                  // CHANGED:
				...
				$seed['error'] = array(                                                           // CHANGED:
					'type' => 'local_state',                                                      // CHANGED:
					'code' => $err,                                                               // CHANGED:
					'hint' => 'No recent server response cached. Run “Check License” to refresh.', // CHANGED:
				);                                                                                // CHANGED:
			}                                                                                    // CHANGED:
		...
		private static function render_plan_usage_card( $last ) {                                  // CHANGED:
			...
			$is_local_seed = ( is_array( $last ) && ! empty( $last['_local_seed'] ) );             // CHANGED:

			$has_plan_data   = ( '' !== (string) $u['plan_slug'] ) || ( (int) $u['max_sites'] > 0 ) || ( true === (bool) $u['unlimited_sites'] ); // CHANGED:
			$has_tokens_data = ( null !== $u['tokens_used'] ) || ( null !== $u['tokens_limit'] ); // CHANGED:
			$has_links_data  = ( '' !== (string) $u['account_url'] ) || ( '' !== (string) $u['upgrade_url'] ); // CHANGED:
			$has_status_data = ( '' !== (string) $u['status'] ) || ( true === (bool) $u['activation_live'] ); // CHANGED:

			$has_any = ( $has_plan_data || $has_tokens_data || $has_links_data || $has_status_data ); // CHANGED:
			if ( ! $has_any ) {                                                                    // CHANGED:
				return;                                                                            // CHANGED:
			}                                                                                      // CHANGED:
			...
			$sites_text    = '';                                                                   // CHANGED:
			$sites_is_mono = false;                                                                // CHANGED:
			if ( $show_sites_row ) {                                                               // CHANGED:
				if ( true === (bool) $u['unlimited_sites'] ) {                                     // CHANGED:
					...
					if ( $max > 0 ) {                                                              // CHANGED:
						...
						if ( true === (bool) $u['activation_live'] && $used <= 1 ) {               // CHANGED:
							...
							$sites_text = sprintf(
								_n( '%d site active', '%d sites active', $used, 'postpress-ai' ),  // CHANGED:
								$used
							);                                                                       // CHANGED:
						} else {                                                                    // CHANGED:
							$sites_text = __( 'No sites active', 'postpress-ai' );                 // CHANGED:
						}                                                                           // CHANGED:
					}                                                                              // CHANGED:
				}                                                                                  // CHANGED:
			}                                                                                      // CHANGED:
