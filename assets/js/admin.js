...
(function () {
  ...
  var PPA_JS_VER = 'admin.v2026-03-17.8'; // CHANGED: one-file remote Save Draft branch in admin.js // CHANGED: clearer Target Audience required notice + focus // CHANGED: add Guide genre title guard (avoid outline item as title) // CHANGED: fix Output Language Original restore + translation loader prepn selection after translation
  ...
  function apiPost(action, payload) {
    ...
    var useJsonBody = (act === 'ppa_generate' || act === 'ppa_store' || act === 'ppa_remote_draft_from_composer'); // CHANGED:
...
// CHANGED:
  ...
  function renderPreview(result, provider) {
    ...
    // and notify the Output Language module so any in-flight translation can't overwrite the fresh preview.
    try {
      ...
      // Dispatch an event to the Output Language module (separate IIFE scope) so it can cancel polling,
      // clear translation cache, and reset its internal state safely.
      try {
        var ev = null;
        if (typeof window.CustomEvent === 'function') {
          ev = new window.CustomEvent('ppa:new_preview', { detail: { ts: Date.now() } });
        } else if (document.createEvent) {
          ev = document.createEvent('CustomEvent');
          ev.initCustomEvent('ppa:new_preview', false, false, { ts: Date.now() });
        }
  ...
  function getSelectedTargetSiteId() {
    var el = $('#postpress-ai-target-site') || $('#ppa-target-site');
    if (!el) return 'current';
    var val = String(el.value || '').trim();
    return val || 'current';
  }

  function isRemoteTargetSiteSelected() {
    var target = getSelectedTargetSiteId();
  ...
  function buildRemoteDraftPayload() {
    ...
    return {
      target_site_id: getSelectedTargetSiteId(),
      post_title: String(payload.title || payload.subject || '').trim(),
      post_content: String(payload.content || '').trim(),
      post_excerpt: String(payload.excerpt || '').trim(),
      post_type: 'post',
      meta: payload.meta || {},
  ...
  function postRemoteDraft(payload) {
    return apiPost('ppa_remote_draft_from_composer', payload);
  }

  function pickRemoteDraftEditLink(body) {
    try {
      if (!body || typeof body !== 'object') return '';
      if (body.edit_link) return String(body.edit_link);
      if (body.remote_edit_link) return String(body.remote_edit_link);
      if (body.link) return String(body.link);

      if (body.data && typeof body.data === 'object') {
        var fromData = pickRemoteDraftEditLink(body.data);
        if (fromData) return fromData;
      }

      if (body.response && typeof body.response === 'object') {
        var fromResp = pickRemoteDraftEditLink(body.response);
        if (fromResp) return fromResp;
      }

      if (body.remote_post && typeof body.remote_post === 'object') {
        var fromPost = pickRemoteDraftEditLink(body.remote_post);
  ...
  function pickRemoteDraftPostId(body) {
    try {
      if (!body || typeof body !== 'object') return '';
      if (body.remote_post_id) return body.remote_post_id;
      if (body.post_id) return body.post_id;
      if (body.id) return body.id;

      if (body.data && typeof body.data === 'object') {
        var fromData = pickRemoteDraftPostId(body.data);
        if (fromData) return fromData;
      }

      if (body.response && typeof body.response === 'object') {
        var fromResp = pickRemoteDraftPostId(body.response);
        if (fromResp) return fromResp;
      }

      if (body.remote_post && typeof body.remote_post === 'object') {
        var fromPost = pickRemoteDraftPostId(body.remote_post);
        if (fromPost) return fromPost;
      }
    } catch (e) {}
    return '';
  ...
  function handleDraftClick(ev) {
    ...
    withBusy(function () {
      if (isRemoteTargetSiteSelected()) {
        var remotePayload = buildRemoteDraftPayload();

        if (!String(remotePayload.post_content || '').trim()) {
          if (draftTab) closeDraftTab(draftTab);
          renderNotice('warn', 'Generate Preview first, then save the remote draft.');
          return Promise.resolve();
        }

        return postRemoteDraft(remotePayload).then(function (res) {
          var wp = unwrapWpAjax(res.body);
          var data = wp.hasEnvelope ? wp.data : res.body;
          var msg = pickMessage(data) || pickMessage(res.body) || 'Remote draft request sent.';

          if (!res.ok || (wp.hasEnvelope && !wp.success)) {
            if (draftTab) closeDraftTab(draftTab);
            renderNotice('error', 'Remote save draft failed (' + res.status + '): ' + msg);
            try { console.info('PPA: remote draft failed', res); } catch (e0) {}
            return;
          }

          var edit = pickRemoteDraftEditLink(data) || pickRemoteDraftEditLink(res.body);
          var pid  = pickRemoteDraftPostId(data) || pickRemoteDraftPostId(res.body);
          var url = absolutizeUrl(edit || '');

          try {
            console.info('PPA: remote draft ok →', {
              edit_link: edit,
              post_id: pid,
              final_url: url,
              popup_handle: !!draftTab,
              popup_name: draftName,
              target_site_id: remotePayload.target_site_id
            });
          } catch (eR) {}
          ...
          if (!draftTab) {
            if (url) {
              renderNoticeTimedHtml(
                'success',
                'Remote draft saved. Pop-up was blocked. <a href="' + escAttr(url) + '" target="_blank" rel="noopener">Open Draft</a>',
                12000
              );
            } else {
              renderNoticeTimed('success', 'Remote draft saved, but no edit link/post ID was returned.', 9000);
            }
            return;
          }

          if (url) {
            var didNavRemote = navigateDraftTarget(draftTab, draftName, url);
            if (didNavRemote) {
              renderNoticeTimed('success', 'Remote draft saved. Opened in a new tab.', 2500);
              return;
            }
          }
    ...
    }, isRemoteTargetSiteSelected() ? 'remote-store' : 'store');
  }
...
(function () {
  ...
  window.PPA_OUTPUT_LANGUAGE_MODULE_INIT = "v2026-02-18.1"; // CHANGED: fix Original restore baseline
  ...
  function freezeOriginalIfNeeded(selectEl, pane) { // CHANGED:
    ...
    // CHANGED: Never re-freeze while the user is viewing a translated preview.
    // This prevents "Original" from accidentally capturing a translated pane as the baseline.
    try {
      if (normalizeLang(state && state.currentLang ? state.currentLang : "original") !== "original") return; // CHANGED:
    } catch (eS) {}
  ...
  function init() {
    ...
    // CHANGED: Best-effort cleanup of any legacy persisted output-language key (safe even if storage is blocked).
    try {
      if (window.localStorage) {
        try { window.localStorage.removeItem("ppa_output_language"); } catch (eLS1) {}
        try { window.localStorage.removeItem("ppa-output-language"); } catch (eLS2) {}
        try { window.localStorage.removeItem("ppa_output_lang"); } catch (eLS3) {}
      }
    } catch (eLS0) {}
    ...
    // CHANGED: When a NEW preview is generated, reset translation state so it cannot overwrite the fresh preview.
    // This is triggered by renderPreview() via a custom event (separate IIFE scope-safe).
    try {
      window.addEventListener("ppa:new_preview", function () {
        try {
          // Cancel any in-flight translation/polling immediately.
          cancelCurrentRequest("new_preview_generated");

          // Reset internal state to a clean baseline for the new preview.
          translationCache.clear();
          state.translating = false;
          state.currentLang = "original";
          ...
          // Keep UI deterministic: new previews always start in Original.
          try { selectEl.value = "original"; } catch (e0) {}

          // If a preview exists, keep controls enabled; MutationObserver will freeze baseline on next tick.
          try { setUiState(selectEl, helpEl, true); } catch (e1) {}
        } catch (e2) {}
      });
    } catch (e3) {}
    ...
    selectEl.addEventListener("change", debounce(function () { // CHANGED:
      ...
      // CHANGED: If the user chooses Original, restore the frozen English baseline FIRST.
      // Important: do NOT call freezeOriginalIfNeeded() before restoring, or we may freeze translated HTML as the baseline.
      if (lang === "original") { // CHANGED:
        restoreOriginal(selectEl, pane); // CHANGED:
        return; // CHANGED:
      } // CHANGED:

      // Always keep frozen original up-to-date before translating.
      freezeOriginalIfNeeded(selectEl, pane); // CHANGED:
