...
(function () {
  ...
  applyOutlineVisibility(); // CHANGED: honor Outline preference on load
  ...
  // ---- Outline toggle + safe links (Preview) ------------------------------ // CHANGED:
  var PPA_OUTLINE_PREF_KEY = 'ppa_show_outline'; // CHANGED:

  function getOutlinePref() { // CHANGED:
    // Default: show outline. Stored as '1' or '0' in localStorage. // CHANGED:
    try { // CHANGED:
      var v = window.localStorage ? window.localStorage.getItem(PPA_OUTLINE_PREF_KEY) : null; // CHANGED:
  ...
  function setOutlinePref(show) { // CHANGED:
    try { // CHANGED:
      if (!window.localStorage) return; // CHANGED:
      window.localStorage.setItem(PPA_OUTLINE_PREF_KEY, show ? '1' : '0'); // CHANGED:
    } catch (e) {} // CHANGED:
  } // CHANGED:

  function safeHref(raw) { // CHANGED:
    // Allow only: http(s), mailto, hash, root-relative. Block javascript/data/etc. // CHANGED:
    var s = String(raw || '').trim(); // CHANGED:
    if (!s) return ''; // CHANGED:
    var low = s.toLowerCase(); // CHANGED:
    if (low.indexOf('javascript:') === 0) return ''; // CHANGED:
    if (low.indexOf('data:') === 0) return ''; // CHANGED:
    if (low.indexOf('vbscript:') === 0) return ''; // CHANGED:
    if (low.indexOf('file:') === 0) return ''; // CHANGED:
    if (low.indexOf('http://') === 0 || low.indexOf('https://') === 0) return s; // CHANGED:
    if (low.indexOf('mailto:') === 0) return s; // CHANGED:
    if (s.charAt(0) === '#' || s.charAt(0) === '/') return s; // CHANGED:
    // If it's a bare domain without scheme, don't auto-link (keeps us safe + predictable). // CHANGED:
    return ''; // CHANGED:
  } // CHANGED:

  function fixLinks(scope) { // CHANGED:
    // Enforce target/_blank + rel on any links rendered into preview (including backend html). // CHANGED:
    if (!scope || scope.nodeType !== 1) return; // CHANGED:
    var links = []; // CHANGED:
    try { links = Array.prototype.slice.call(scope.querySelectorAll('a') || []); } catch (e1) { links = []; } // CHANGED:
    for (var i = 0; i < links.length; i++) { // CHANGED:
      var a = links[i]; // CHANGED:
      if (!a || a.nodeType !== 1) continue; // CHANGED:
      var href = ''; // CHANGED:
      try { href = a.getAttribute('href') || ''; } catch (e2) { href = ''; } // CHANGED:
      var safe = safeHref(href); // CHANGED:
      if (!safe) { // CHANGED:
        try { a.removeAttribute('href'); } catch (e3) {} // CHANGED:
        continue; // CHANGED:
      } // CHANGED:
  ...
  function applyOutlineVisibility(pane) { // CHANGED:
    var show = getOutlinePref(); // CHANGED:
    var scope = pane || getPreviewPane(); // CHANGED:
    if (!scope || scope.nodeType !== 1) return; // CHANGED:

    // Preferred: wrapper we generate. // CHANGED:
    var wraps = []; // CHANGED:
    try { wraps = Array.prototype.slice.call(scope.querySelectorAll('.ppa-outline-wrap,[data-ppa-outline-wrap]') || []); } catch (e1) { wraps = []; } // CHANGED:
    for (var i = 0; i < wraps.length; i++) { // CHANGED:
      try { wraps[i].style.display = show ? '' : 'none'; } catch (e2) {} // CHANGED:
    } // CHANGED:

    // Fallback: hide outline block + its "Outline" heading if wrapper doesn't exist. // CHANGED:
    var blocks = []; // CHANGED:
    try { blocks = Array.prototype.slice.call(scope.querySelectorAll('.ppa-outline') || []); } catch (e3) { blocks = []; } // CHANGED:
    for (var j = 0; j < blocks.length; j++) { // CHANGED:
      var b = blocks[j]; // CHANGED:
      if (!b || b.nodeType !== 1) continue; // CHANGED:
      try { b.style.display = show ? '' : 'none'; } catch (e4) {} // CHANGED:
      try { // CHANGED:
        var prev = b.previousElementSibling; // CHANGED:
        if (prev && String(prev.tagName || '').toUpperCase() === 'H3' && /outline/i.test(String(prev.textContent || ''))) { // CHANGED:
          prev.style.display = show ? '' : 'none'; // CHANGED:
        } // CHANGED:
      } catch (e5) {} // CHANGED:
    } // CHANGED:
  } // CHANGED:

  function ensureOutlineToggleControl() { // CHANGED:
    // Inserts a small checkbox next to the main action buttons. // CHANGED:
    var id = 'ppa-outline-toggle'; // CHANGED:
    if (document.getElementById(id)) return; // CHANGED:

    // Prefer to insert after Generate button. // CHANGED:
    var anchor = btnGenerate || btnPreview || btnDraft || btnPublish; // CHANGED:
    if (!anchor || !anchor.parentNode) return; // CHANGED:

    var wrap = document.createElement('span'); // CHANGED:
    wrap.id = 'ppa-outline-toggle-wrap'; // CHANGED:
    wrap.className = 'ppa-outline-toggle-wrap'; // CHANGED:
    try { // CHANGED:
      wrap.style.marginLeft = '10px'; // CHANGED:
      wrap.style.whiteSpace = 'nowrap'; // CHANGED:
      wrap.style.display = 'inline-flex'; // CHANGED:
      wrap.style.alignItems = 'center'; // CHANGED:
      wrap.style.gap = '6px'; // CHANGED:
    } catch (e0) {} // CHANGED:

    var cb = document.createElement('input'); // CHANGED:
    cb.type = 'checkbox'; // CHANGED:
    cb.id = id; // CHANGED:
    cb.checked = getOutlinePref(); // CHANGED:

    var label = document.createElement('label'); // CHANGED:
    label.setAttribute('for', id); // CHANGED:
    label.textContent = 'Show Outline'; // CHANGED:
    try { label.style.cursor = 'pointer'; } catch (e1) {} // CHANGED:

    wrap.appendChild(cb); // CHANGED:
    wrap.appendChild(label); // CHANGED:

    // Insert after anchor button. // CHANGED:
    try { // CHANGED:
      if (anchor.nextSibling) anchor.parentNode.insertBefore(wrap, anchor.nextSibling); // CHANGED:
      else anchor.parentNode.appendChild(wrap); // CHANGED:
    } catch (e2) { // CHANGED:
      try { root.appendChild(wrap); } catch (e3) {} // CHANGED:
    } // CHANGED:

    cb.addEventListener('change', function () { // CHANGED:
      setOutlinePref(!!cb.checked); // CHANGED:
      applyOutlineVisibility(); // CHANGED:
    }); // CHANGED:
  } // CHANGED:
  ...
  function markdownToHtml(md) {
    ...
    txt = txt.replace(/\[([^\]]+)\]\(([^\)]+)\)/g, function (_m, label, url) {        // CHANGED:
      var href = safeHref(url);                                                             // CHANGED:
      if (!href) return label;                                                              // CHANGED:
      return '<a href="' + escAttr(href) + '">' + label + '</a>';                           // CHANGED:
    });                                                                                     // CHANGED:

    // Bare URLs (http/https) — conservative match to avoid weird edge cases.              // CHANGED:
    txt = txt.replace(/(https?:\/\/[\w\-._~:/?#\[\]@!$&'()*+,;=%]+)(?![^<]*>)/g, function (_m, url) { // CHANGED:
      var href2 = safeHref(url);                                                            // CHANGED:
      if (!href2) return url;                                                               // CHANGED:
      return '<a href="' + escAttr(href2) + '">' + url + '</a>';                            // CHANGED:
    });                                                                                     // CHANGED:
