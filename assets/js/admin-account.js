...
2026-02-28: FIX: Support Chat calls Django/Render directly (CORS) + parses agent_message schema + removes non-allowed headers. // CHANGED:
*/
...
(function () {
  ...
  function renderFromData(data) {
    ...
    // Prefer the email that the LICENSE KEY is connected to.

    var billingEmail =(license && (license.billing_email || license.email)) ||
...
// Tokens
  ...
  // ----------------------------
  // Support Chat (direct to Django/Render)
  // ----------------------------
  var chatState = {
    open: false,
    busy: false,
  ...
  function bestSupportChatUrl() {
    var cfg = window.PPAAccount || {};
    var u =
      cfg.supportChatUrl ||
      cfg.support_chat_url ||
      (window.PPA && (window.PPA.supportChatUrl || window.PPA.support_chat_url)) ||
      (window.ppaAdmin && (window.ppaAdmin.supportChatUrl || window.ppaAdmin.support_chat_url)) ||
      '';

    u = toSafeStr(u).trim();
    if (!u) u = 'https://apps.techwithwayne.com/postpress-ai/support/chat/';

    if (u.indexOf('?') === -1 && u.slice(-1) !== '/') u = u + '/';
    return u;
  }

  async function postSupportChatDirect(params) {
    var url = withTs(bestSupportChatUrl());
    ...
    try {
      ...
      var text = await resp.text();
      var json = null;
      try { json = JSON.parse(text); } catch (e0) { json = null; }
      ...
      return {
        ...
        meta: {
          ...
          http_status: 0,
          endpoint: 'support_chat',
          reason: String((e1 && e1.message) ? e1.message : e1)
        }
      };
    }
  }
  ...
  function parseChatReplyPayload(payload) {
    ...
    var acts =
      (Array.isArray(core.suggested_actions) ? core.suggested_actions :
      Array.isArray(core.suggestedActions) ? core.suggestedActions :
  ...
  async function sendSupportChatMessage(message) {
    ...
    var payload = await postSupportChatDirect(params);
  ...
  function handleSuggestedAction(actionId) {
    ...
    if (id === 'buy_tokens') {
      ...
      fetchIntent('buy_tokens').then(function (payload) {
        var url = extractLinkFromPayload(payload || lastPayload, 'buy_tokens', getHref($('ppa-account-buy-tokens')));
        if (!url) {
          try { popup.close(); } catch (e) {}
  ...
  function bindSupportChatEventsOnce() {
    ...
    document.addEventListener('click', function (e) {
      ...
      if (t.closest && t.closest('#ppa-support-chat-send')) {
        ...
        var input = $('ppa-support-chat-input');
        var val = input ? toSafeStr(input.value) : '';
        if (input) input.value = '';
        if (toSafeStr(val).trim()) {
